...
void obstaclesCallback(const iarc7_msgs::ObstacleArray::ConstPtr& msg)
{
  ...
  for (auto &obstacle : msg->obstacles) {
    ...
    float px, py, pz;
    for (pz = 0; pz <= pipe_height; pz += 0.005) {
      for (float theta = 0; theta < 2*M_PI; theta += 0.05){
        px = pipe_radius*cos(theta) + pipe_x;
        py = pipe_radius*sin(theta) + pipe_y;
  ...
  for (float z = 0; z <= 5; z += 0.05) {
    for (float i = 0; i <= 20; i += 0.05) {
      ...
      point1.x = -10; point1.y = -10 + i;
      point2.x = -10 + i; point2.y = 10;
      point3.x = 10; point3.y = 10 - i;
      point4.x = 10 - i; point4.y = -10;
...
int main(int argc, char ** argv){
  ...
  // Assumes map was retrieved. Check for initialization first
  ...
  //Initialize planner
  // Fix to get params from IARC param list
